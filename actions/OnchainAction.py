...
class OnchainAction:
    ...
    def processOnchainTokens(self, cookie: str) -> bool:
        ...
        try:
            ...
            logger.info(f"Found {len(onchainTokens)} tokens at time {datetime.now()}")
            # Persist to database and get successfully persisted tokens
            persistedTokens = self.persistTokens(onchainTokens)
            logger.info(f"Persisted {len(persistedTokens)} tokens at time {datetime.now()}")
    ...
    def persistTokens(self, onchainTokens: List[OnchainInfo]) -> List[OnchainInfo]:
        """
        Persist OnchainInfo objects to database using optimized batch processing
        ...
        """
        try:
            logger.info(f"Starting batch persistence of {len(onchainTokens)} tokens at time {datetime.now()}")
            
            # Use the new batch insertion method
            successfulTokens = self.db.onchain.batchInsertTokens(onchainTokens)
            
            logger.info(f"Batch persisted {len(successfulTokens)} tokens at time {datetime.now()}")
            
            if successfulTokens:
                try:
                    # Use the optimized notification processing for better performance
                    sentCount = OnchainNotificationStrategies.sendNotificationOptimized(
                        tokensNeedsToBeProcessed=successfulTokens,
                        db=self.db,
                        notificationManager=self.notificationManager
                    )
                    logger.info(f"Optimized processing: {len(successfulTokens)} tokens for notifications, sent {sentCount} notifications")
                    ...
                    # try:
                    #     logger.info("Falling back to original notification processing")
                    #     sentCount = OnchainNotificationStrategies.sendNotification(
                    #         tokensNeedsToBeProcessed=successfulTokens,
                    #         db=self.db,
                    #         notificationManager=self.notificationManager
                    #     )
                    #     logger.info(f"Fallback processing: sent {sentCount} notifications")
                    # except Exception as fallback_error:
                    #     logger.error(f"Both optimized and fallback notification processing failed: {str(fallback_error)}")
                        
            logger.info(f"Successfully batch persisted {len(successfulTokens)} onchain tokens")
