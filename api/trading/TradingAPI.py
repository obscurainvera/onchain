...
def addToken():
    ...
    try:
        ...
        if not validate_solana_address(token_address):
            return jsonify({
                'success': False,
                'error': 'Invalid tokenAddress format (must be 44 characters, base58)'
            }), 400
        
        if not validate_solana_address(pair_address):
            return jsonify({
                'success': False,
                'error': 'Invalid pairAddress format (must be 44 characters, base58)'
            }), 400
...
@trading_bp.route('/api/tokens/health', methods=['GET', 'OPTIONS'])
def getSystemHealth():
    """
    System Health API - GET /api/tokens/health
    ...
    try:
        ...
        logger.error(f"Error in getSystemHealth API: {str(e)}", exc_info=True)
        return jsonify({
            'success': False,
            'error': f'Internal server error: {str(e)}'
        }), 500